...
export default function Header() {
  ...
  return (
    <div className="sticky top-0 z-50 w-full transition-all duration-300">
      <div className={`transition-all duration-500 overflow-hidden ${isScrolled ? 'h-0 opacity-0' : 'h-auto opacity-100'}`}>
        <TopbarCountdown />
      </div>
      <header className={`w-full transition-all duration-500 ease-in-out border-b ${
        ...
        <div className="text-2xl font-bold text-[#1F83C6] uppercase tracking-wide">
          <Link href="/"><img className={`transition-all duration-500 ease-in-out ${isScrolled ? 'w-32' : 'w-42'}`} src="/images/logo.png" alt="" /></Link>
        </div>
